...
# `audited-pass` at HEAD is a claim being made by THIS turn. Each such row must
# have an audit record (written by record-audit.sh, i.e. by the auditor when it
# actually finished) whose verdict is `pass` and whose code-hash equals the
# current audit-hash. So:
#
...
#
# Rows already `audited-pass` at HEAD are not re-checked: they got past this gate
# to be committed in the first place, and re-verifying them against a hash that
# every later slice legitimately moves would fail every board forever.
#
...
# The board as it stands at HEAD. Absent (first slice, or board just added) means
# every audited-pass row is new and must be backed by a record.
prev=""
if git cat-file -e "HEAD:$board" 2>/dev/null; then
  tmp_prev="$(mktemp)"
...
while IFS="$(printf '\t')" read -r id status; do
  ...
  # Already audited-pass at HEAD -> already went through this gate. Not re-checked.
